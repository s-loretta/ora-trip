...
export default function AccountDashboard() {
  ...
  // ⚡️ NOUVEAU : FONCTION DE MISE À JOUR DU PROFIL
  const handleUpdateProfile = async (fieldKey: string, newValue: string) => {
    try {
      // Met à jour la propriété spécifique (ex: { first_name: "Nouveau" })
      await sdk.store.customer.update({ [fieldKey]: newValue });
      window.location.reload(); // Rafraîchit pour resynchroniser Zustand avec la base de données
    } catch (error: any) {
        ...
        <div className="min-h-[50vh] relative">
          <AnimatePresence mode="wait">
           ...
           {activeTab === 'commandes' && (
            ...
            {/* 5. RÉGLAGES (MAINTENANT INTERACTIFS) */}
              ...
              <motion.div key="parametres" variants={containerVariants} initial="hidden" animate="visible" exit="exit" className="flex flex-col gap-20 pt-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-20">
                  <div className="flex flex-col gap-0">
                    ...
                    {/* ⚡️ Utilisation du nouveau InfoRow interactif */}
                    <InfoRow 
                      label="Prénom" 
...
// ⚡️ INFOROW INTELLIGENT (INLINE EDIT)
...
const InfoRow = ({ label, value, editable = false, fieldKey, type = "text", onSave }: InfoRowProps) => {
  const [isEditing, setIsEditing] = useState(false);
  const [inputValue, setInputValue] = useState(value);
  const [isSaving, setIsSaving] = useState(false);

  const handleSave = async () => {
    // Évite de sauvegarder si la valeur n'a pas changé ou si c'est vide
    if (!fieldKey || !onSave || inputValue === value) {
      setIsEditing(false);
      return;
    }
      ...
      <div className="flex flex-col gap-2 w-full md:w-2/3">
        ...
        {isEditing ? (
          <input
            ...
            disabled={isSaving}
            className="bg-transparent border-b border-white py-1 text-lg font-light text-white focus:outline-none focus:ring-0 disabled:opacity-50"
          />
        ) : (
          <span className="text-lg font-light text-white">
            {value || <span className="text-light-grey/40 italic text-sm">Non renseigné</span>}
          </span>
        )}
      </div>
          ...
          {isEditing ? (
            <>
              <button 
                onClick={handleSave} 
                disabled={isSaving} 
                className="text-[9px] uppercase tracking-[0.2em] text-white hover:text-light-grey/80 transition-colors disabled:opacity-50"
              >
                {isSaving ? "[ Patientez... ]" : "[ Enregistrer ]"}
              </button>
              <button 
                onClick={() => { setIsEditing(false); setInputValue(value); }} 
                disabled={isSaving} 
                className="text-[9px] uppercase tracking-[0.2em] text-white/40 hover:text-white transition-colors"
              >
                [ Annuler ]
              </button>
            </>
            ...
            <button 
              ...
              className="text-[9px] uppercase tracking-[0.2em] text-white/20 group-hover:text-white/80 transition-colors"
            >
              [ Modifier ]
            </button>
          )}
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            <label className="uppercase text-light-grey/40 tracking-widest text-[9px]">Complément d'adresse</label>
